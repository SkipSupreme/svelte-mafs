...
At initialization, the residual block's $F$ is small (especially with the GPT-2 init scaling from the previous lesson — its output projection is initialized near zero). So $\partial F/\partial x \approx 0$, and
