...
The fix on the count side is **add-$s$ smoothing**: pretend every cell got $s$ extra observations before normalizing. Every probability becomes nonzero; the rare ones become small but finite. Crank the **smoothing** slider up — the count panel visibly flattens. At $s = 20$, every cell is close to uniform.
