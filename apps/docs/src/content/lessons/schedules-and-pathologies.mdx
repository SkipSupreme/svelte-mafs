...
nanoGPT, GPT-3, PaLM, Llama — all use warmup followed by cosine decay. It's become the reference schedule for transformer training.
</Step>
...
So "reaching the global minimum" isn't the goal. *Which* zero-loss minimum you reach determines how the model generalizes. Flat, wide minima generalize well. Sharp, narrow minima generalize badly. SGD's noise (Lesson 10.2) biases convergence toward flat minima — that's a big part of why SGD-trained models generalize at all.
...
# AdamW — so weight decay is decoupled from the adaptive step.
