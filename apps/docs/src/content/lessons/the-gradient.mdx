...
$f$ has 2 inputs and 1 output. Its **graph** is a surface in 3D. Its **gradient** is a vector in 2D — it has the same number of components as the input. The gradient is NOT an arrow on the tilted surface. It lives *in the input plane*, next to the contour lines.
