...
1. **At GPT-2 small's scale, the embedding alone is 31% of the total.** That's a quirk of small models with large vocabularies. As $d$ grows, $V \cdot d$ stays linear in $d$ but $N \cdot 12 d^2$ goes quadratic — embeddings shrink as a share of the budget.
